...
sys.path.append('./')
from src.diffusers_model_pipeline import CustomizationAttnProcessor, LaTexBlendXLPipeline, set_use_memory_efficient_attention_xformers
from src.diffusers_data_pipeline import CustomDiffusionDataset, PromptDataset, collate_fn
from diffusers.models.lora import LoRACompatibleLinear
from src.src_unet2dconditionmodel import UNet2DConditionModel
    ...
    def change_attn(unet):
        for layer in unet.children():
            if type(layer) is Attention:
                ...
                setattr(layer, 'to_1k', LoRACompatibleLinear(layer.cross_attention_dim, layer.inner_dim, bias=False).half())
                setattr(layer, 'to_1v', LoRACompatibleLinear(layer.cross_attention_dim, layer.inner_dim, bias=False).half())
                layer.to_1k.weight.data = layer.to_k.weight.data.clone().half()
                layer.to_1v.weight.data = layer.to_v.weight.data.clone().half()

            else:
                change_attn(layer)

    change_attn(unet)
    unet.set_attn_processor(CustomizationAttnProcessor())
    return unet
...
def encode_prompt(text_encoders, tokenizers, prompt, text_input_ids_list=None):
    ...
    for i, text_encoder in enumerate(text_encoders):
        ...
        pooled_prompt_embeds = prompt_embeds[0]   #  prompt_embeds: BaseModelOutputWithPooling, len=3
        prompt_embeds = prompt_embeds.hidden_states[-2]  # prompt_embeds: tuple
        bs_embed, seq_len, _ = prompt_embeds.shape
        prompt_embeds = prompt_embeds.view(bs_embed, seq_len, -1)
        prompt_embeds_list.append(prompt_embeds)

    prompt_embeds = torch.concat(prompt_embeds_list, dim=-1)  # torch.Size([bs, 77, 2048])
    pooled_prompt_embeds = pooled_prompt_embeds.view(bs_embed, -1) # torch.Size([bs, 1280])
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help="The prompt for replacing specifying the instance",
    )
    parser.add_argument(
        "--class_prompt",
        type=str,
    ...
    args.instance_prompt = template.format(args.instance_prompt)
    args.replace_prompt = template.format(args.replace_prompt) """
...
def main(args):
    ...
    # Handle replacement prompt.
    replace_time_ids = compute_time_ids()
    if not args.train_text_encoder and args.modifier_token is None and args.replace_prompt is not None:
        replace_prompt_hidden_states, replace_pooled_prompt_embeds = compute_text_embeddings(
            args.replace_prompt, text_encoders, tokenizers
        )
    ...
    if args.replace_prompt:
        add_time_ids = torch.cat([add_time_ids, replace_time_ids], dim=0)
    ...
    if not args.train_text_encoder and args.modifier_token is None:
        ...
        if args.replace_prompt:
            prompt_embeds = torch.cat([prompt_embeds, replace_prompt_hidden_states], dim=0)
            unet_add_text_embeds = torch.cat([unet_add_text_embeds, replace_pooled_prompt_embeds], dim=0)

    else:
        print('pass encoding')
        pass
        """ args.instance_prompt = np.random.choice(template_list).format(args.instance_prompt)
        ...
        if args.replace_prompt:
            args.replace_prompt = np.random.choice(template_list).format(args.replace_prompt)
            replace_tokens_one = tokenize_prompt(tokenizer_one, args.replace_prompt)
            replace_tokens_two = tokenize_prompt(tokenizer_two, args.replace_prompt)
            tokens_one = torch.cat([tokens_one, replace_tokens_one], dim=0)
            tokens_two = torch.cat([tokens_two, replace_tokens_two], dim=0) """
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if args.train_text_encoder or args.modifier_token is not None:
            ...
            text_encoder_one.module.text_model.embeddings.requires_grad_(True)
            text_encoder_two.module.text_model.embeddings.requires_grad_(True)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                timesteps = timesteps.long()    # tensor([417, 308], device='cuda:1')
                ...
                if not args.train_text_encoder and args.modifier_token is None:
                    ...
                    full_instance_prompt = np.random.choice(template_list).format(args.instance_prompt)
                    tokens_one = tokenize_prompt(tokenizer_one, full_instance_prompt)
                    tokens_two = tokenize_prompt(tokenizer_two, full_instance_prompt)
                    if args.with_prior_preservation:
                        class_tokens_one = tokenize_prompt(tokenizer_one, args.class_prompt)
                        class_tokens_two = tokenize_prompt(tokenizer_two, args.class_prompt)
                        tokens_one = torch.cat([tokens_one, class_tokens_one], dim=0)
                        tokens_two = torch.cat([tokens_two, class_tokens_two], dim=0)
                    if args.replace_prompt:
                        full_replace_prompt = np.random.choice(template_list).format(args.replace_prompt)
                        replace_tokens_one = tokenize_prompt(tokenizer_one, full_replace_prompt)
                        replace_tokens_two = tokenize_prompt(tokenizer_two, full_replace_prompt)
                        tokens_one = torch.cat([tokens_one, replace_tokens_one], dim=0)
                        tokens_two = torch.cat([tokens_two, replace_tokens_two], dim=0)
                    ...
                    pos_info = {}
                    pos_info["ins_start"] = list(tokens_two[0]).index(torch.tensor(49408))
                    pos_info["ins_end"] = list(tokens_two[0]).index(torch.tensor(49407))
                    concept_len = pos_info["ins_end"] - pos_info["ins_start"]
                    pos_info["replace_end"] = list(tokens_two[-1]).index(torch.tensor(49407))
                    pos_info["replace_start"] = pos_info["replace_end"] - concept_len
                    prompt_embeds_input_dict = {}
                    prompt_embeds_input_dict['pos_info'] = pos_info
                    prompt_embeds_input_dict['prompt_embeds_input'] = prompt_embeds_input 

                    model_pred = unet(
                        noisy_model_input, timesteps, prompt_embeds_input_dict, added_cond_kwargs=unet_added_conditions
                    ).sample
    ...
    if accelerator.is_main_process:
        ...
        text_encoder_one = accelerator.unwrap_model(text_encoder_one) if isinstance(text_encoder_one,
                                                                                    torch.nn.parallel.DistributedDataParallel) else text_encoder_one
        text_encoder_two = accelerator.unwrap_model(text_encoder_two) if isinstance(text_encoder_two,
                                                                                    torch.nn.parallel.DistributedDataParallel) else text_encoder_two
        pipeline = LaTexBlendXLPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
            unet=accelerator.unwrap_model(unet),
